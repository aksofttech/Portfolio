...
void main() {
  ...
  // Fade out smoothly towards the very edges of the canvas to remove the "box" clipping effect
  vec2 uv = gl_FragCoord.xy / uResolution.xy;
  float edgeFadeX = smoothstep(0.0, 0.2, uv.x) * (1.0 - smoothstep(0.8, 1.0, uv.x));
  float edgeFadeY = smoothstep(0.0, 0.2, uv.y) * (1.0 - smoothstep(0.8, 1.0, uv.y));
  c *= edgeFadeX * edgeFadeY;
...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const dpr = Math.min(window.devicePixelRatio, 1); // Reduced DPR for performance
