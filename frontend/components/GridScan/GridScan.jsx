...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float hitIsY = 1.0;
    ...
    {
        float isY = float(i < 2);
        float pos = mix(-1.0, 1.0, float(i)) * isY + mix(-2.0, 2.0, float(i - 2)) * (1.0 - isY);
  ...
  if (uLineStyle > 0.5) {
    ...
    float edgeDistX = min(abs(hit.x - (-2.0)), abs(hit.x - 2.0));
    float edgeDistY = min(abs(hit.y - (-1.0)), abs(hit.y - 1.0));
    float edgeDist = mix(edgeDistY, edgeDistX, hitIsY);
    float edgeGate = 1.0 - smoothstep(gridScale * 2.0, gridScale * 8.0, edgeDist);
    altMask *= edgeGate;
...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    renderer.setPixelRatio(Math.min(window.devicePixelRatio || 1, 1)); // Reduced DPR for performance
